...
class Pencere3(QMainWindow):
    ...
    def setUI(self):
        self.setGeometry(600,100,200,75)
    ...
    def anaWidget(self):
        ...
        self.skor = QLabel()
        self.h_box.addWidget(self.skor)
        widget.setLayout(self.h_box)
...
########################################################################################################################
    ...
    def anaWidget(self):
        ...
        yazi1 = QLabel("Kaç Dakika:")
        yazi1.setMaximumWidth(150)
        self.sürecubugu = QLineEdit()
        ...
        self.h_box.addWidget(self.sürecubugu)
...
#########################################################################################################################
    def yeni_kelime_function(self):
        ...
        if (self.puan <= 120):
            if (self.kelime.zorluk == 1):
                self.random_kelime.setText(self.kelime.ingilizce)
            else:
                self.yeni_kelime_function()
        elif (self.puan <= 250 and self.puan > 120):
            if (self.kelime.zorluk == 2):
                self.random_kelime.setText(self.kelime.ingilizce)
            else:
                self.yeni_kelime_function()
        elif (self.puan <= 400 and self.puan > 250):
            if (self.kelime.zorluk == 3):
                self.random_kelime.setText(self.kelime.ingilizce)
            else:
                self.yeni_kelime_function()
        elif (self.puan <= 700 and self.puan > 400):
            if (self.kelime.zorluk == 4):
                self.random_kelime.setText(self.kelime.ingilizce)
            else:
                self.yeni_kelime_function()
        elif (self.puan >= 1000 and self.puan > 700):
            if (self.kelime.zorluk == 5):
                self.random_kelime.setText(self.kelime.ingilizce)
            else:
                self.yeni_kelime_function()
        else:
    ...
    def gonder_function(self):
        if (self.cevap.text() == self.kelime.türkçe):
            ...
            self.pencere3.skor.setText("Süre Bitti Toplam Puanınız : " + str(self.puan))
            self.now = time.time()
            if (self.now > self.future):
                self.pencere3.show()
            else:
                self.yeni_kelime_function()
    ...
    def getir_function(self):
        self.puan = 0
        self.now = time.time()
        self.future = self.now + int(self.sürecubugu.text()) * 60
        kelime_listesi = sonsözlük.kütüphane.verileri_al()
        self.kelime_listesi_tmp = kelime_listesi
        self.kelime_sayisi = len(self.kelime_listesi_tmp)
        self.yeni_kelime_function()
        self.pencere3.skor.setText(str("Süre Bitti Toplam Puanınız : " + str(self.puan)))
